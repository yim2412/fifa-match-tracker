...
앱 상단 **자동 수집** 체크박스로 켜고 끈다. 켜면 Windows 작업 스케줄러에
`피파전적관리 자동수집` 작업이 등록되고, 6시간마다 `collect.py`가 조용히 돌면서
등록 계정의 새 경기를 쌓는다(콘솔 창이 안 뜨게 `pythonw.exe`로 실행).

**왜 필요한가** — 조회할 때만 쌓이면 하루 앱을 안 켠 사이에 100경기가 밀려나 뚫린다.
6시간 주기면 20시간 창에 여유가 충분하다.

- **PC가 켜져 있어야 한다.** 꺼져 있는 동안은 안 쌓인다
- 체크 상태는 앱이 아니라 **실제 스케줄러가 정답** — 앱 밖에서 지우면 체크도 풀린다
- 뭘 했는지는 `.cache/collect.log` 에 남는다
- 관리자 권한은 필요 없다(현재 사용자 작업)

```powershell
python collect.py            # 등록 계정 전부 수동 수집
...
- [x] 공격력·수비력·기대득점률·선방력 — `PlayerStat` 자체 공식 (API 미제공, 직접 계산).
      선방력은 `defending` 스탯 그대로라 참고 사이트와 거의 일치. 나머지는 자체 가중치라 값이 다름
- [ ] 상대 구단주별 상성 분석
- [ ] 기간별 승률 추이 차트
- [x] 조회 결과 로컬 DB(SQLite) 누적 — `store.py`
- [x] `collect.py` + 작업 스케줄러 — 앱 체크박스로 껐다 켠다
- [ ] 선수 이미지 (넥슨 CDN)
- [ ] exe 빌드 (PyInstaller)
