...
- **저장**: 조회할 때마다 로컬 SQLite(`fifa.db`)에 누적 — API가 최근 100경기까지만 주는 한계를 넘기 위해
...
| 탭 / 기능 | 내용 |
|-----------|------|
| 흐름 분석 | 집계를 문장으로 — 최근 흐름과 이기는/지는 방식을 글로 설명 |
| 전적 | 최근 경기 목록 — 결과·스코어·상대·점유율·슛 등 |
| 선수 지표 | 선수별 출전·평점·공격력·수비력·기대득점률·선방력(공식 미제공 값 역산) |
| 전술·경기 결과 | 상대 전술(포메이션)별 승률, 결과 분포 |
| 상대 전적 | 구단주별 상성, 포지션별 최다 상대 |
| 승률 추이 | 기간별 승률·등급(디비전) 계단 그래프 |
| 기간별 추이 | 1일/2일/1주/1개월 단위 전적 + 연승·연패 기록 |
| 시즌별 성적 | 감독모드 랭킹 시즌별 전적·승률·평균 득실 비교 |
| 승부처 분석 | 선제골 승률, 시간대별 득실 분포, 시각대별 승률 |
| 성적 진단 | 상대 등급(디비전)별 승률·평균 득실, 점유율 구간별 승률 |
| 선수별 결정력 | 슛·유효슛·골·전환율·xG·어시스트 랭킹 |
| 선수 조합 | 함께 선발로 나온 선수쌍별 승률 — 잘 맞는/안 맞는 조합 |
| 슛 맵 | 슛 좌표를 하프 피치에 시각화(내 슛 / 상대 슛 실점 위치) |
| 선수 카드 | 능력치·특성·시세·클럽경력 + 강화단계·적응도·팀컬러 시뮬레이터 |
| 랭커 카드 | 감독모드 순위·구단가치·랭킹점수(ELO) |

## 화면

닉네임을 검색하면 랭커 카드로 요약을 보여주고, "감독모드 분석"에서 탭별 상세로 들어간다.

| | |
|---|---|
| **랭커 카드** — 순위·점수·전적·구단가치 요약<br>![랭커 카드](docs/screenshots/ranker.png) | **선수 지표** — 포지션별 출전·평점·공수 지표(공식 미제공 값 역산)<br>![선수 지표](docs/screenshots/players.png) |
...
| **승부처 분석** — 선제골 승률, 시간 구간별 득실, 시각대별 승률<br>![승부처 분석](docs/screenshots/clutch.png) | **슛 맵** — 슛 좌표를 하프 피치에 시각화(골/유효슛/빗나감)<br>![슛 맵](docs/screenshots/shotmap.png) |
...
| 파일 | 역할 |
|------|------|
| `app_main.py` | PyQt6 UI — 검색 바, 랭커 카드, 분석 탭 16종(흐름 분석·선수 지표·전술·승부처 분석·성적 진단·슛 맵·결정력·선수 조합·팀컬러 등), 조회 워커 스레드 |
| `widgets.py` | 화면 부품(위젯) — 랭커 카드·표·승률/등급 그래프·축구장 스쿼드 배치(`PitchWidget`)·슛 맵·`NoScrollComboBox` 등 |
| `theme.py` | 색상 상수·전역 스타일시트(QSS) — 배경·승/패·수비 등 색과 버튼·글꼴 스타일을 한 곳에 |
...
- [x] 선수 조합(케미) 탭 — 함께 선발(SUB·GK 제외)로 나온 선수쌍별 승률
      (`stats.pair_synergy`). 최소 경기수 필터, 승률 50% 기준 색칠(초록/빨강),
      선수 더블클릭 시 카드. 누적 전체 기준
...
넥슨 오픈API·데이터센터를 이용하는 **비공식 개인 프로젝트**다. EA SPORTS FC 온라인과
관련 데이터의 권리는 NEXON / EA 에 있으며, 이 프로젝트는 넥슨·EA 와 무관하다.
