...
- [x] 슛 맵 — `shootDetail`의 x/y 좌표를 공격 진영 하프 피치 위에 점으로
      ...
      골대맞음은 흰 테두리. "슛 맵" 탭에서 내 슛/상대 슛(실점 위치) 토글, 표시 구간
      기준. 요약(슛·골·유효슛률·전환율) 함께 표시
- [x] 자동 새로고침 — 표시 범위 바의 체크박스로 켜면 앱이 켜져 있는 동안 N분
      (기본 `config.AUTO_REFRESH_MIN`=5)마다 "새 경기 확인"을 자동 실행. 새 경기가
      있을 때만 화면을 갱신하고, 없으면 보던 탭·범위를 건드리지 않고 상태바로만
      확인 시각을 남긴다. 트레이 상주 없음(API가 소급 조회돼 백그라운드 불필요)
