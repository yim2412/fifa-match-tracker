...
- **인코딩은 지금 전부 명시돼 있다 — 그 상태를 유지한다.** 2026-08-15 전수 점검에서 이 프로젝트는
  `open()`·`read_text()`·`write_text()` 전부 `encoding="utf-8"` 이었고 위험 지점이 0건이었다.
  일반 규칙은 전역 `~/.claude/CLAUDE.md`. 이 프로젝트에서 특히 지킬 것:
  전적 데이터를 CSV 로 내보내는 기능을 추가하면 **`encoding="utf-8-sig"`** 를 쓴다
  (Excel 이 BOM 없는 UTF-8 을 못 알아봐 선수명 한글이 깨진다).
  닉네임·구단명은 전부 한글이라 여기서 실수하면 바로 눈에 보이는 손상이 된다.
