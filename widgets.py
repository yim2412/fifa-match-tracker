...
class FitTableWidget(QTableWidget):
    ...
    안 남고, 좁으면(다 안 들어가면) 원래 너비 그대로 두고 가로 스크롤이 뜬다.
    """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._content_widths: dict[int, int] = {}

    def set_content_widths(self, widths: dict[int, int]) -> None:
        self._content_widths = widths
        self._stretch_to_fill()

    def resizeEvent(self, event) -> None:
        super().resizeEvent(event)
        self._stretch_to_fill()

    def _stretch_to_fill(self) -> None:
        widths = self._content_widths
        if not widths:
            return
